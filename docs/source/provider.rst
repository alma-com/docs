...
Помощники - это пользовательские классы, которые расширяют функционал сайта.

Пример помощника можно найти ``/app/helpers/alma.p``. Данный класс содержит один метод ``num_decline``, который позволяет склоненить имена существительных после числительных.